...
// =========================================================================
// --- TYPE SAFE UTILITIES FOR TEXT EXTRACTION ---
// Defining these locally ensures we don't depend on broken library exports
// and avoids 'any' which causes build failures.
// =========================================================================
...
async function extractTextFromPdf(pdf: MinimalPdfDocument): Promise<string> {
  let fullText = "";
  ...
  for (let i = 1; i <= numPages; i++) {
    try {
      ...
      // Type Guard to safely filter items
...
export default function PdfViewer({ documentId, currentPage, onPageChange }: PdfViewerProps) {
  ...
  // Using explicit 'unknown' and casting to our interface avoids implicit 'any' error
  const onDocumentLoadSuccess = useCallback(async (pdf: unknown) => {
    // Cast to our defined interface so TypeScript knows the structure
    const pdfDocument = pdf as MinimalPdfDocument;

    setViewState(prev => ({ ...prev, numPages: pdfDocument.numPages }));
    ...
    try {
      const extractedText = await extractTextFromPdf(pdfDocument);
      setPdfText(extractedText);
      console.log("PDF Text Extracted on Frontend.");
    } catch (e) {
